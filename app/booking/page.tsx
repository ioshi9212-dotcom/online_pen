...
export default async function BookingPage({ searchParams }: { searchParams: { client?: string; service?: string; time?: string; busy?: string } }) {
  ...
  const grouped = free.reduce((map, item) => {
    const key = dayKey(item.startAt);
    map.set(key, [...(map.get(key) || []), item]);
    return map;
  }, new Map<string, typeof free>());

  const selectedSlot = searchParams.time
    ? free.find((w) => w.startAt.toISOString() === searchParams.time)
    : undefined;
  ...
  return (
    <main className="booking-page">
      ...
      <section className="step-block">
        <h2>1. Услуга</h2>
        {services.length === 0 ? <div className="empty-state">Активных услуг пока нет.</div> : null}
          ...
          <div className="window-list">
            {Array.from(grouped.entries()).slice(0, 14).map(([key, items]) => (
              <div className="window-row" key={key}>
                ...
                <div className="time-pills">
                  {items.map((w) => (
                    <a key={w.id} href={`/booking?client=${token}&service=${selectedService.id}&time=${encodeURIComponent(w.startAt.toISOString())}#confirm`}>
                      {fmtTime(w.startAt)}
                    </a>
                  ))}
                </div>
            ...
            {free.length === 0 ? (
              <div className="empty-state">
                <p>Свободных окон нет. Можно встать в лист ожидания в кабинете.</p>
                <a className="button secondary" href={`/my?client=${token}#waitlist`}>В лист ожидания</a>
              </div>
            ) : null}
          </div>
        </section>
        ...
        <section className="step-block" id="confirm">
          ...
          <form action={createBooking} className="grid" style={{ marginTop: 16 }}>
            ...
            <div className="actions">
              <button type="submit">Подтвердить и отправить заявку</button>
              <a className="button secondary" href={`/booking?client=${token}&service=${selectedService.id}`}>Выбрать другое время</a>
            </div>
          </form>
        </section>
      ) : null}
