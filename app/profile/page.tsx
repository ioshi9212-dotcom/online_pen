  ...
  return (
    <header className="client-topbar">
      ...
      <nav>
        ...
        <a href={`/booking?client=${token}`}>Записаться</a>
      </nav>
...
export default async function ClientProfilePage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <main className="client-shell">
      <ClientMenu token={profileId} name={client.firstName} />

      <section className="client-welcome">
        <div>
          <p className="client-eyebrow">Профиль</p>
      ...
      {searchParams.saved ? <div className="notice ok-notice floating-toast">Профиль сохранён.</div> : null}
      {searchParams.error === "phone-exists" ? <div className="notice danger-notice floating-toast">Такой телефон уже есть у другого клиента.</div> : null}
      {searchParams.error === "required" ? <div className="notice danger-notice floating-toast">Заполните обязательные поля.</div> : null}

      <form action={updateClientProfile} className="client-card client-form">
        ...
        <label>Фото профиля по ссылке<input name="avatarUrl" defaultValue={client.avatarUrl} placeholder="https://..." /></label>
        <div className="actions">
          <button type="submit">Сохранить профиль</button>
          <a className="client-button secondary" href={`/my?client=${profileId}`}>Назад в кабинет</a>
        </div>
      </form>
    </main>
  );
